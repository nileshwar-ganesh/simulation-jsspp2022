...
public class AlgBFPGL {
	...
	public AlgBFPGL(int totalMachines) {
			...
			if(mIndex < 0) {
				...
				if(!isBackFillable) {
					...
					if(isSchedulable) {
						...
						int jobStartTime = Integer.max(jobReleaseTime, machines.get(mIndexL).getAvailableTime());
						jobCompletionTime = jobStartTime + jobProcessingTime;
						//System.out.println(jobCompletionTime);
						for(int c=0; c<jobCore; c++) {
							if(machines.get(mIndexL+c).getAvailableTime() < jobStartTime) {
								MachineB machine = new MachineB();
								machine.createSlot(machines.get(mIndexL+c).getMachineID(), machines.get(mIndexL+c).getAvailableTime(), jobStartTime);
